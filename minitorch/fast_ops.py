    ...
    * When `out`, `a`, `b` are stride-aligned, avoid indexing
    ...
    def _zip(
        ...
        if list(a_strides) == list(b_strides) == list(out_strides) and list(a_shape) == list(b_shape) == list(out_shape):
            ...
            for i in prange(len(out)):  # Main loop in parallel
                out_index: Index = np.zeros(MAX_DIMS, np.int32)  # All indices use numpy buffers
                a_index: Index = np.zeros(MAX_DIMS, np.int32)  # All indices use numpy buffers
                b_index: Index = np.zeros(MAX_DIMS, np.int32)  # All indices use numpy buffers
                to_index(i, out_shape, out_index)
                # o = index_to_position(out_index, out_strides)
